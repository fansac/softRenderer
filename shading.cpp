...
void phong_shading_shadow(mesh::TriangleMesh& mesh, rst::Rasterizer& r) {
	...
	s.set_model_transformation(0, 1);
	s.set_view_volume(45, -4, -5);
	//s.set_camera(light_position, -light_position.normalized(), Eigen::Vector3d(0, 1, 0));
	s.set_camera({ 0, 0, 5 }, {0, 0, -1}, Eigen::Vector3d(0, 1, 0));
	s.calculate_matrix();
		...
		for (auto x = x_min; x <= x_max; ++x) {
			for (auto y = y_min; y <= y_max; ++y) {
				if (tri.is_inside({ static_cast<double>(x), static_cast<double>(y) })) {
					...
					double z_view = 1.0 / (alpha / w[0] + beta / w[1] + gamma / w[2]);
					double z_depth_correct = (alpha * p0.z() / w[0] + beta * p1.z() / w[1] + gamma * p2.z() / w[2]) * z_view;
					if (s.compare_pixel_in_z_buffer(x, y, s.to_z_buffer_value(z_depth_correct))) {
						s.draw_pixel({ x, y, Eigen::Vector3d(0, 0, 1) * 255 });
					}
				}
			}
		}
	...
	for (auto iter : s.z_buffer) {
		shadow_map[index++] = static_cast<double>(iter) / static_cast<double>(UINT16_MAX) * 255.0;
	}
	...
	system("pause");
	cv::Mat t_img(WINDOW_HEIGHT, WINDOW_WIDTH, CV_64FC3, s.canvas.data());
	t_img.convertTo(t_img, CV_8UC3, 1.0f);
	cv::cvtColor(t_img, t_img, cv::COLOR_RGB2BGR);
	cv::flip(t_img, t_img, 0);
	//CV_WINDOW_NORMAL
	cv::namedWindow("test_draw_line", CV_WINDOW_NORMAL);
	cv::imshow("test_draw_line", t_img);
	cv::waitKey();
