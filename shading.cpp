...
void gouraud_shading(mesh::TriangleMesh& mesh, rst::Rasterizer& r) {
	...
	for (const auto& iter : mesh.triangles) {
		...
		for (unsigned int i = 0; i < 3; ++i) {
			...
			Eigen::Vector3d position_world = util_rd::homo_to_v3(r.m_model * position_i.homogeneous());
...
void normal_map_shading(mesh::TriangleMesh& mesh, rst::Rasterizer& r, tex::Texture& tex, tex::Texture& normal_map) {
	std::vector<Eigen::Vector3d> light_position = { { -20, 0, 20 } };
	Eigen::Vector3d light_intensity = { 500, 500, 500 };
	Eigen::Vector3d ambient_light_intensity = { 20, 20, 20 };
	Eigen::Vector3d ka = { 0.002, 0.002, 0.002 };
			...
			for (auto y = y_min; y <= y_max; ++y) {
				if (tri.is_inside({ static_cast<double>(x), static_cast<double>(y) })) {
					...
					if (r.compare_pixel_in_z_buffer(x, y, r.to_z_buffer_value(z_depth_correct))) {
						...
						double v = (alpha * texcoord[0][1] / w[0] + beta * texcoord[1][1] / w[1] + gamma * texcoord[2][1] / w[2]) * z_view;
						Eigen::Vector3d n = ((alpha * normals[0] / pv0.z() + beta * normals[1] / pv1.z() + gamma * normals[2] / pv2.z()) * z_view).normalized();
						Eigen::Vector3d t = ((alpha * tangents[0] / pv0.z() + beta * tangents[1] / pv1.z() + gamma * tangents[2] / pv2.z()) * z_view);
						t = (t - n * t.dot(n)).normalized();
						Eigen::Vector3d b = n.cross(t);
						
						Eigen::Matrix3d TBN;
						TBN.col(0) = t, TBN.col(1) = b, TBN.col(2) = n;
						Eigen::Vector3d normal = (normal_map.get_color(tex::texcoord_wrap(u), tex::texcoord_wrap(v)) / 255.0 * 2.0 - Eigen::Vector3d(1, 1, 1));
						normal = TBN * normal;
						normal.normalize();
						//normal = n;
						//light_pos_view
...
void displacement_map_shading(mesh::TriangleMesh& mesh, rst::Rasterizer& r, tex::Texture& tex, tex::Texture& normal_map) {
	std::vector<Eigen::Vector3d> light_position = { { 15, 15, 15 }};
	Eigen::Vector3d light_intensity = { 500, 500, 500 };
	Eigen::Vector3d ambient_light_intensity = { 10, 10, 10 };
	Eigen::Vector3d ka = { 0.002, 0.002, 0.002 };
	...
	for (const auto& iter : mesh.triangles) {
		...
		for (unsigned int i = 0; i < 3; ++i) {
			...
			double u = texcoord[i][0];
			double v = texcoord[i][1];

			double dhdu = kh * ((normal_map.get_height(tex::texcoord_wrap(u + 1.0 / static_cast<double>(tex.width)), tex::texcoord_wrap(v)) - normal_map.get_height(tex::texcoord_wrap(u - 1.0 / static_cast<double>(tex.width)), tex::texcoord_wrap(v))) / 2.0 * static_cast<double>(tex.width));
			double dhdv = kh * ((normal_map.get_height(tex::texcoord_wrap(u), tex::texcoord_wrap(v + 1.0 / static_cast<double>(tex.height))) - normal_map.get_height(tex::texcoord_wrap(u), tex::texcoord_wrap(v - 1.0 / static_cast<double>(tex.height)))) / 2.0 * static_cast<double>(tex.height));

			Eigen::Vector3d tu = t + kn * dhdu * n;
			Eigen::Vector3d tv = b + kn * dhdv * n;

			normals[i] = (tu.cross(tv)).normalized();
			points_in_view[i] = points_in_view[i] + kn * kh * normal_map.get_height(tex::texcoord_wrap(u), tex::texcoord_wrap(v)) * normals[i];

			// (pixel.x, pixel.y) in [WIDTH, HEIGHT]
			auto homo = r.m_vp * r.m_per * points_in_view[i].homogeneous();
			w[i] = homo.w();
			pixels[i] = util_rd::homo_to_v3(homo);
			// convert z depth value from [-1, 1] to [far, near];
			pixels[i].z() = f1 * pixels[i].z() + f2;
